...
var Swiper = function Swiper(node) {
  ...
  var root = typeof node === 'string' ? $(node) : node;
  var eventHub = {
    'swipLeft': [],
    'swipRight': []
  };
...
function () {
  ...
  _createClass(Player, [{
    ...
    value: function loadSong() {
      ...
      this.audio.src = songObj.url; // 当audio对象加载了元数据的时候! 将进度条的结束时间设置为当前audio的持续时间赋值给当前的area-time的innerText!
    ...
    value: function locateLyric() {
      ...
      var nextLineTime = this.lyricArr[this.lyricIndex + 1][0]; // 如果当前的时间大于下一行的时间 或者说当前的歌词坐标小于歌词数组的长度减一的话!

      if (currentTime > nextLineTime && this.lyricIndex < this.lyricArr.length - 1) {
        this.lyricIndex++;
        var node = this.$("[data-time=\"" + this.lyricArr[this.lyricIndex][0] + "\"]");
    ...
    value: function setLyrics(lyrics) {
      ...
      var lyricArr = [];
      this.lyricArr = lyricArr; // 歌词通过回车将其分割成字符串数组 并且通过过滤器进行匹配! 并且竟可能少的匹配换行符之外的字符!
    ...
    value: function setLyricsToCenter(node) {
      ...
      this.$$(".panel-lyrics p").forEach(function (element) {
    ...
    value: function setProgerssBar() {
      ...
      var percent = this.audio.currentTime * 100 / this.audio.duration + "%"; //并且将百分比给定当前的 progress类的样式width属性中!

      this.$(".bar .progress").style.width = percent; //与此同时 将当前的时间格式化之后 播放进度条里面 最前面就是time-start也就是当前的播放时间!
    ...
    value: function formateTime(secondsTotal) {
      //将其转换成为分钟!
      var minutes = parseInt(secondsTotal / 60); //如果minutes大于或者说是等于10的话 那么就将值转换成为字符串 否则小于10前缀加0然后转换成为字符串!

      minutes = minutes >= 10 ? "" + minutes : "0" + minutes; //如果是秒钟的话对60取余操作! 最后面将两者拼接起来返回出去!
